...
// Helper function to recursively set active state in tree (outside component to avoid dependency issues)
function setActiveInTree(item: SidebarMenuItem, activeItemId: string): SidebarMenuItem {
  const updatedItem = { ...item, isActive: item.id === activeItemId };
  
  if (item.children) {
    updatedItem.children = item.children.map(child => 
      setActiveInTree(child, activeItemId)
    );
  }
...
export function SidebarMenuProvider({ children }: SidebarMenuProviderProps) {
  ...
  // Initialize menu data with default active item
  const [menuData, setMenuData] = useState<SidebarSection[]>(() => {
    // Set default item as active on initialization
    return sidebarData.map(section => ({
      ...section,
      items: section.items.map(item => 
        setActiveInTree(item, 'default')
      )
    }));
  });
  ...
  }, [navigationCallback, getNavigationInfo]);
